...
/////////
//ERASE SEARCH -> precio_desde

    if(isset($_SESSION["id_precio"]) && $_SESSION["id_precio"] != ""){
        $sql_inmueble_des = mysql_query("SELECT * FROM precios WHERE `id_precios` = ".$_SESSION['id_precio_val']);
        $row_inmueble_des = mysql_fetch_array($sql_inmueble_des);
        
?>                    
                    <a href="<?=$_SERVER['PHP_SELF']?>?id_precios=0&inicio=<?=$_SESSION['inicio']?>&fin=<?=$_SESSION['fin']?>" style="font-size:12px;text-decoration:none;color:#212121;">

                        <font color="gray">Desde:</font>
                        <br/>
                        <table width="100%" align="center" style="background-color:#D3D3D3;border-radius:3px;font-size:12px;padding:2px;text-shadow:0 1px 1px #fff;cursor:pointer;margin-bottom:5px;">
                            <tbody>
                                <tr>
...
while($row_inmueble = mysql_fetch_array($sql_inmueble)){
	$sql_inmueble_tot = mysql_query("SELECT COUNT(*) FROM inmuebles i, datos f WHERE i.status = 'Activa' and i.cod_inmueble = f.cod_inmueble $query AND i.tipo_operacion = '".$row_inmueble[0]."'");
...
<?php
while($row_inmueble = mysql_fetch_array($sql_inmueble)){
	$sql_inmueble_tot = mysql_query("SELECT COUNT(*) FROM inmuebles i, datos f WHERE i.status = 'Activa' and i.cod_inmueble = f.cod_inmueble $query AND i.tipo_inmueble = '".$row_inmueble[0]."'");
	$row_inmueble_tot = mysql_fetch_array($sql_inmueble_tot);
...
<?php
while($row_inmueble = mysql_fetch_array($sql_inmueble)){
	$sql_inmueble_tot = mysql_query("SELECT COUNT(*) FROM inmuebles i, datos f WHERE i.status = 'Activa' and i.cod_inmueble = f.cod_inmueble $query AND i.idp = ".$row_inmueble[0]);
	$row_inmueble_tot = mysql_fetch_array($sql_inmueble_tot);
...
while($row_inmueble = mysql_fetch_array($sql_inmueble)){
	$sql_inmueble_tot = mysql_query("SELECT COUNT(*) FROM inmuebles i, datos f WHERE i.status = 'Activa' and i.cod_inmueble = f.cod_inmueble $query AND i.iddep = ".$row_inmueble[0]);
	$row_inmueble_tot = mysql_fetch_array($sql_inmueble_tot);
...
<?php
while($row_inmueble = mysql_fetch_array($sql_inmueble)){
	$sql_inmueble_tot = mysql_query("SELECT COUNT(*) FROM inmuebles i, datos f WHERE i.status = 'Activa' and i.cod_inmueble = f.cod_inmueble $query AND i.id_distritos = ".$row_inmueble[0]);
	$row_inmueble_tot = mysql_fetch_array($sql_inmueble_tot);
...
if($_SESSION["cod_oficina"] == ""){
 ...
 if($_SESSION["id_precio"] != "")
        $query .= " AND id_precio = ".$_SESSION["id_precio_val"];
...
<?php
while($row_inmueble = mysql_fetch_array($sql_inmueble)){
	$sql_inmueble_tot = mysql_query("SELECT COUNT(*) FROM inmuebles i, datos f WHERE i.status = 'Activa' and i.cod_inmueble = f.cod_inmueble $query AND i.cod_oficina1 = '".$row_inmueble[0]."'");
	$row_inmueble_tot = mysql_fetch_array($sql_inmueble_tot);
...
<?php
while($row_inmueble = mysql_fetch_array($sql_inmueble)){
    $sql_inmueble_tot = mysql_query("SELECT COUNT(*) FROM inmuebles i, datos f WHERE i.status = 'Activa' and i.cod_inmueble = f.cod_inmueble $query AND i.cod_agente_captador_oficina1 = '".$row_inmueble[0]."'");
    $row_inmueble_tot = mysql_fetch_array($sql_inmueble_tot);
...
if($_SESSION["id_agente"] == ""){
 ...
 if($_SESSION["id_precio"] != "")
        $query .= " AND id_precio = ".$_SESSION["id_precio_val"];
    $sql_inmueble = mysql_query("SELECT DISTINCT(precio) FROM precios  where id_precios not in (2) order by descripcion");
    $row_inmueble = mysql_num_rows($sql_inmueble);
    if($row_inmueble > 0){ ?>
                    <br />
                    ...
                    <table width="100%  " border="0" align="center" cellpadding="0" cellspacing="0" style="padding-top:5px;">
                        <tbody>
                            <tr>
                                <td>
                                    <form action="" method="get" onChange="submit()" style="width:186px;font-family:Times New Roman;font-size:16px;">
                                        <select name="id_precio" id="id_precio" style="border-radius:5px;padding:3px;font-size:14px;color:#333333;width:175px;">
                                            <option value=''>-</option>

<?php
while($row_inmueble = mysql_fetch_array($sql_inmueble)){
    $sql_inmueble_tot = mysql_query("SELECT COUNT(*) FROM precios WHERE  precio = '".$row_inmueble[0]."'");
    $row_inmueble_tot = mysql_fetch_array($sql_inmueble_tot);
    
    $sql_inmueble_des = mysql_query("SELECT * FROM precios WHERE precio = ".$row_inmueble[0]);
    $row_inmueble_des = mysql_fetch_array($sql_inmueble_des);
    //$desde=$row_inmueble_des[1];
...
if($_SESSION["id_precio"] != ""){
    $query .= " AND precio = ".$_SESSION["id_precio_val"];
}
